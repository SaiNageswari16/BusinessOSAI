      ...
      <div className="container mx-auto px-4 space-y-20">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="relative h-[250px] rounded-lg overflow-hidden group cursor-pointer shadow-sm">
            ...
            <div className="absolute inset-0 p-8 flex flex-col justify-center">
              <span className="text-blue-600 font-bold mb-2 tracking-wide uppercase text-sm">Tech Sale</span>
              <h3 className="text-3xl font-black text-white mb-4 leading-tight">Latest<br/>Electronics</h3>
              <span className="text-white font-bold underline hover:text-blue-600 transition-colors text-sm tracking-wider">SHOP NOW</span>
            </div>
          </div>
          <div className="relative h-[250px] rounded-lg overflow-hidden group cursor-pointer shadow-sm">
            ...
            <div className="absolute inset-0 p-8 flex flex-col justify-center text-right items-end">
              <span className="text-blue-600 font-bold mb-2 tracking-wide uppercase text-sm">New Arrivals</span>
        ...
        <section>
          <div className="flex justify-between items-center mb-8">
            ...
            <div className="flex space-x-2">
              <button className="w-8 h-8 rounded-full border border-[#E5E4E2] flex items-center justify-center hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-colors"><ChevronLeft className="w-4 h-4" /></button>
              <button className="w-8 h-8 rounded-full border border-[#E5E4E2] flex items-center justify-center hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-colors"><ChevronRight className="w-4 h-4" /></button>
            </div>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
            {categories.map((cat, idx) => (
              <div key={idx} className="flex flex-col items-center group cursor-pointer">
                <div className="w-32 h-32 rounded-full overflow-hidden border-4 border-white shadow-md mb-4 group-hover:border-blue-600 transition-colors">
                ...
                <h4 className="font-bold text-[#1A1A1A] group-hover:text-blue-600 transition-colors">{cat.name}</h4>
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-10">
            {tabOptions.map(tab => (
              <button 
                ...
                className={`font-bold uppercase tracking-wider pb-2 border-b-2 transition-colors ${activeTab === tab ? 'text-blue-600 border-blue-600' : 'text-gray-500 border-transparent hover:text-[#1A1A1A]'}`}
              >
                {tab}
              </button>
            ))}
            ...
            <div className="flex space-x-6 mt-4 md:mt-0">
              {["Special", "New", "Bestseller"].map(tab => (
                <button 
                  ...
                  className={`font-bold uppercase text-sm tracking-wider transition-colors ${ourProductsTab === tab ? 'text-blue-600' : 'text-gray-500 hover:text-[#1A1A1A]'}`}
                >
                  {tab}
                </button>
              ))}
        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8">
            <div className="flex justify-between items-center mb-6">
              ...
              <Link to="/store" className="text-sm font-bold text-gray-500 hover:text-blue-600 uppercase">View All</Link>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {[
                ...
                <div key={idx} className="group cursor-pointer border border-[#E5E4E2] rounded-lg overflow-hidden bg-white hover:shadow-md transition-shadow">
                  ...
                  <div className="p-6">
                    <div className="text-sm text-blue-600 font-bold mb-2">{blog.date}</div>
                    <h4 className="text-lg font-bold text-[#1A1A1A] group-hover:text-blue-600 transition-colors mb-3 line-clamp-1">{blog.title}</h4>
                    ...
                    <span className="text-sm font-bold text-[#1A1A1A] uppercase tracking-wider group-hover:text-blue-600 border-b-2 border-transparent group-hover:border-blue-600 transition-all">Read More</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <section className="border-t border-b border-[#E5E4E2] py-8">
           <div className="flex justify-between items-center opacity-60 grayscale hover:grayscale-0 transition-all duration-500">
             {mockVendors.slice(0,5).map(v => (
               <div key={v.id} className="text-2xl font-black text-gray-400 uppercase tracking-widest text-center cursor-pointer hover:text-blue-600">
                 {v.name.split(' ')[0]}
               </div>
             ))}
           </div>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 py-6">
          <div className="flex items-center p-4 bg-white rounded-lg group">
            <div className="w-14 h-14 rounded-full border border-[#E5E4E2] flex items-center justify-center text-blue-600 group-hover:bg-blue-600 group-hover:text-white transition-colors mr-4 flex-shrink-0">
