    ...
    <div className="bg-white min-h-screen pb-20">
      ...
      <div className="bg-[#F2F2F2] py-10 mb-12 border-b border-[#E5E4E2]">
        <div className="container mx-auto px-4 text-center">
          <h1 className="text-3xl font-black text-[#1A1A1A] uppercase tracking-widest mb-4">Shop</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-1 space-y-8">
            ...
            <div className="border border-[#E5E4E2] rounded-lg p-6">
              <h3 className="text-lg font-black text-[#1A1A1A] uppercase tracking-wide mb-4">Categories</h3>
              <ul className="space-y-3">
                <li onClick={() => setSelectedCategory(undefined)} className="flex justify-between items-center group cursor-pointer">
                    <span className={`transition-colors ${!selectedCategory ? 'text-blue-600 font-bold' : 'text-gray-600 group-hover:text-blue-600'}`}>All</span>
                </li>
                {categories.map(cat => (
                  <li key={cat.id} onClick={() => setSelectedCategory(cat.id)} className="flex justify-between items-center group cursor-pointer">
                    <span className={`transition-colors ${selectedCategory === cat.id ? 'text-blue-600 font-bold' : 'text-gray-600 group-hover:text-blue-600'}`}>{cat.name}</span>
            ...
            {/* Price Filter (Mock) */}
            <div className="border border-[#E5E4E2] rounded-lg p-6">
              <h3 className="text-lg font-black text-[#1A1A1A] uppercase tracking-wide mb-4">Filter By Price</h3>
              <div className="mb-4">
              ...
              <div className="flex justify-between items-center">
                ...
                <button className="bg-[#1A1A1A] hover:bg-blue-600 text-white px-4 py-1.5 text-xs font-bold uppercase rounded transition-colors">Filter</button>
              </div>
            </div>

            {/* Brands */}
            <div className="border border-[#E5E4E2] rounded-lg p-6">
              <h3 className="text-lg font-black text-[#1A1A1A] uppercase tracking-wide mb-4">Brands</h3>
              <ul className="space-y-3">
          ...
          <div className="lg:col-span-3">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center bg-[#F2F2F2] p-4 border border-[#E5E4E2] rounded-lg mb-8">
              <div className="flex items-center space-x-4 mb-4 md:mb-0">
                ...
                <span className="text-sm text-gray-500">Showing 1–{allProducts.length} of {allProducts.length} results</span>
              </div>
