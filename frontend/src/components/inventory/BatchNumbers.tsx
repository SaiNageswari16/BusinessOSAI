        ...
        <table className="w-full text-sm text-left">
          <thead className="bg-muted/50 text-muted-foreground text-xs uppercase font-semibold">
            <tr>
              ...
              <th className="px-6 py-4">Mfg / Expiry</th>
            ...
            {filtered.map((batch) => (
              <tr key={batch.id} className="hover:bg-muted/30 transition-colors">
                ...
                <td className="px-6 py-4">
                  <div className="text-xs text-muted-foreground">Mfg: {batch.mfgDate}</div>
                  <div className="text-xs font-semibold flex items-center gap-1 mt-0.5"><CalendarClock className="size-3 text-rose-500" /> Exp: {batch.expDate}</div>
                </td>
