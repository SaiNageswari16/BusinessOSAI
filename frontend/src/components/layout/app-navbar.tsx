...
export function AppNavbar() {
      ...
      const authorizedItems = group.items
        ...
        .map(item => {
          ...
          const authorizedSubItems = item.subItems.filter(sub => {
            if (sub.to.includes("global_users")) return isPlatformAdmin;
  ...
  // Active group logic - strip query params from item.to before matching
  const getBasePath = (to: string) => to.split('?')[0].replace(/\/$/, "");
  const normalizedPath = currentPath.replace(/\/$/, "");
  
  const activeGroup =
    authorizedNav.find(g =>
      g.items.some(item =>
        normalizedPath === getBasePath(item.to) ||
        item.subItems?.some((sub) =>
          normalizedPath === getBasePath(sub.to) ||
          (normalizedPath.startsWith('/erp') && sub.to === '/erp?tab=companies')
        )
      )
    ) ||
    authorizedNav.find(g =>
      g.items.some(item =>
        (getBasePath(item.to).length > 1 && normalizedPath.startsWith(getBasePath(item.to))) ||
        item.subItems?.some((sub) => getBasePath(sub.to).length > 1 && normalizedPath.startsWith(getBasePath(sub.to)))
      )
    ) ||
    authorizedNav.find(g => (g.group === 'Sales & CRM' && normalizedPath.includes('/crm'))) ||
    authorizedNav[0];
  return (
    <div className="flex flex-col shrink-0 bg-card border-b border-border z-10 shadow-sm relative w-full">
      {/* Level 1: Main Modules */}
      <nav className="flex items-center gap-1 px-6 overflow-x-auto scrollbar-none h-[80px]">
        {authorizedNav.map((g) => {
      ...
      {/* Level 2: Categories (Secondary Bar) */}
                ...
                if (item.subItems && item.subItems.length > 0) {
                  ...
                  return (
                    <DropdownMenu key={item.label}>
                      ...
                      <DropdownMenuContent align="start" className="w-56 max-h-96 overflow-y-auto p-1.5">
