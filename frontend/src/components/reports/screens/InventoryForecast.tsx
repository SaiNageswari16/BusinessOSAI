...
export function InventoryForecast() {
  ...
  useEffect(() => {
    const fetchReport = async () => {
      try {
        const data = await inventoryApi.getReportData('inventory_forecast');
        setReportData(data);
      } catch (err) {
        console.error(err);
      } finally {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="bg-gradient-to-r from-emerald-500/10 to-transparent border-emerald-500/20 border border-border/50 rounded-3xl p-6 lg:p-8 shadow-sm relative overflow-hidden">
