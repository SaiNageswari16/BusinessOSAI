...
export function AbcAnalysisReports() {
  ...
  useEffect(() => {
    const fetchReport = async () => {
      try {
        const data = await inventoryApi.getReportData('abc_analysis_reports');
        setReportData(data);
      } catch (err) {
        console.error(err);
      } finally {
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="bg-card border-2 border-border/60 rounded-3xl p-6 flex flex-col h-full min-h-[200px]">
        ...
        <div className="mt-6 pt-4 border-t border-border/40">
          ...
          <div className="bg-card border border-border/50 rounded-3xl p-5 flex flex-col gap-2 h-full">
            <h3 className="text-xs font-bold uppercase tracking-wider mb-2 px-2 text-muted-foreground">Key Metrics</h3>
