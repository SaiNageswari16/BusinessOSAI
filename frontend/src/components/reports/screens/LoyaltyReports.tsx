...
export function LoyaltyReports() {
  ...
  useEffect(() => {
    const fetchReport = async () => {
      try {
        const data = await inventoryApi.getReportData('loyalty_reports');
        setReportData(data);
      } catch (err) {
        console.error(err);
      } finally {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="bg-gradient-to-r from-sky-500/10 to-transparent border-sky-500/20 border border-border/50 rounded-3xl p-6 lg:p-8 shadow-sm relative overflow-hidden">
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          <div className="flex flex-col gap-4">
            {metrics.slice(0,3).map((metric: any, idx: number) => {
              ...
              return (
      <motion.div key={idx} initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.1 * idx }} className={"relative overflow-hidden rounded-3xl p-5 bg-card border-l-4 min-h-[140px] flex flex-col justify-between " + (metric.isPositive ? "border-l-emerald-500 shadow-[0_0_20px_rgba(16,185,129,0.1)]" : "border-l-red-500 shadow-[0_0_20px_rgba(239,68,68,0.1)]")}>
        <div className="absolute -right-2 -top-2 text-foreground/5 pointer-events-none"><Icon className="w-24 h-24" /></div>
        <p className="text-sm font-medium text-muted-foreground mb-1 truncate relative z-10">{metric.label}</p>
        <div className="relative z-10">
          <h3 className="text-2xl lg:text-3xl font-bold text-foreground truncate">{metric.value}</h3>
          <p className={"text-xs mt-1 truncate " + (metric.isPositive ? "text-emerald-500" : "text-red-500")}>{metric.change}</p>
        </div>
      </motion.div>
    );
            })}
