...
export function AttritionPredictionReports() {
  ...
  useEffect(() => {
    const fetchReport = async () => {
      try {
        const data = await inventoryApi.getReportData('attrition_prediction_reports');
        setReportData(data);
      } catch (err) {
        console.error(err);
      } finally {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="space-y-6">
        <div className="bg-card border border-border/50 rounded-3xl p-6 shadow-sm h-[350px] md:h-[450px]">
      ...
      <div className="w-full h-full min-h-[300px]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {metrics.map((metric: any, idx: number) => {
            ...
            return (
      <motion.div key={idx} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-card rounded-3xl p-5 shadow-xl shadow-black/5 border border-border/30 min-h-[140px] flex flex-col justify-between">
        <Icon className="w-8 h-8 text-primary mb-2 shrink-0" />
        <div>
          <h3 className="text-2xl font-extrabold text-foreground truncate">{metric.value}</h3>
          <p className="text-xs text-muted-foreground font-medium mt-1 truncate">{metric.label}</p>
        </div>
      </motion.div>
    );
          })}
      ...
      <div className="bg-gradient-to-r from-indigo-500/10 to-transparent border-indigo-500/20 border border-border/50 rounded-3xl p-6 lg:p-8 shadow-sm relative overflow-hidden">
