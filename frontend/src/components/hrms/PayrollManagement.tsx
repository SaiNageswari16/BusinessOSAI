...
export function PayrollManagement({ tab = "salary_structure" }: Props) {
  ...
  if (tab === "payroll_processing") {
    const selectedEmpForProcess = employees.find(e => e.id === processEmpId);
    const existingStructForProcess = structures.find(s => s.employee_id === processEmpId);
    const processBasic = existingStructForProcess ? existingStructForProcess.basic_salary : (selectedEmpForProcess?.basic_salary ? parseFloat(String(selectedEmpForProcess.basic_salary)) : 45000);
    const calculatedProcess = calculateStatutory(processBasic);
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          <div>
            <h2 className="text-2xl font-bold tracking-tight text-foreground flex items-center gap-2">
              Payroll Processing <span className="text-xs px-2.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-500 font-semibold flex items-center gap-1"><Sparkles className="size-3" /> Fully Synced</span>
            </h2>
            <p className="text-xs text-muted-foreground">Automated monthly payroll cycles, statutory compliance & digital payslip disbursement.</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            >
              <Zap className="size-3.5" /> ⚡ Run 1-Click Batch Payroll (All Active)
            </button>
            <button
              onClick={() => {
                if (employees.length > 0 && !processEmpId) setProcessEmpId(employees[0].id);
                setProcessDialogOpen(true);
              }}
              className="flex items-center gap-1.5 px-3 h-8.5 bg-card border border-border/80 hover:bg-muted/50 text-foreground rounded-lg text-xs font-semibold shadow-sm transition-colors"
            >
              <CreditCard className="size-3.5 text-primary" /> Process Single Employee
            </button>
          </div>
        </div>

        {/* Processing checklist & live stats */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="glass-panel p-6 rounded-xl border border-border/50 space-y-4 md:col-span-2">
            <div className="flex justify-between items-center">
              <h3 className="font-semibold text-foreground flex items-center gap-2">
                <Shield className="size-4 text-primary" /> Automated Payroll Compliance Checklist
              </h3>
            ...
            {[
              { step: `Configured salary structures for all active team members (${structures.length}/${employees.length})`, done: structures.length >= employees.length || employees.length === 0 },
              { step: "Standard EPFO 12% PF & ESIC 0.75% deductions auto-applied", done: true },
              { step: "Statutory tax TDS calculations synchronized", done: true },
              { step: `Vault electronic payslip generation & employee sync (${payslips.length} issued)`, done: payslips.length > 0 },
            ].map((item) => (
              <div key={item.step} className="flex items-center gap-3 text-sm">
                ...
                <span className={item.done ? "text-foreground font-medium" : "text-muted-foreground"}>{item.step}</span>
              </div>
            ))}
          </div>

          <div className="glass-panel p-5 rounded-xl border border-border/50 flex flex-col justify-center space-y-3">
            <div className="flex justify-between items-center">
            ...
            <div className="flex justify-between items-center">
              ...
              <p className="text-lg font-bold text-primary">{payslips.length}</p>
            </div>
            <div className="pt-2 border-t border-border/50">
              <button
                onClick={() => setBatchDialogOpen(true)}
                className="w-full py-2 bg-primary/10 hover:bg-primary/20 text-primary rounded-lg text-xs font-bold transition-colors flex items-center justify-center gap-1.5"
              >
                <Zap className="size-3.5" /> Disburse Monthly Payroll
              </button>
            </div>
          </div>
        </div>

        {/* Process Single Payroll Dialog */}
        {processDialogOpen && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="w-full max-w-lg rounded-2xl bg-card border p-6 space-y-4 shadow-2xl max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="font-bold text-lg text-foreground">Process Employee Payroll</h3>
                  <p className="text-xs text-muted-foreground">Calculate earnings, statutory deductions, and issue a digital salary slip.</p>
                </div>
                <span className="text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded bg-primary/10 text-primary">Live Sync</span>
              </div>
              ...
              <form onSubmit={handleProcessPayroll} className="space-y-4">
                <div className="space-y-1">
                  ...
                  <select
                    value={processEmpId}
                    onChange={e => setProcessEmpId(e.target.value)}
                ...
                {processEmpId && (
                  <div className="p-3.5 bg-muted/40 rounded-xl border border-border/60 space-y-2.5">
                    <div className="flex justify-between items-center">
                      <span className="text-xs font-semibold text-foreground">Calculated Compensation Preview</span>
                      <span className="text-[10px] text-emerald-500 font-bold bg-emerald-500/10 px-2 py-0.5 rounded-full flex items-center gap-1">
                        <Check className="size-2.5" /> Auto-Derived
                      </span>
                    </div>
                    <div className="grid grid-cols-3 gap-2 text-center text-xs">
                      <div className="p-2 bg-background/80 rounded border">
                        ...
                        <p className="font-bold text-foreground">{currency.symbol}{calculatedProcess.gross.toLocaleString()}</p>
                      </div>
                      <div className="p-2 bg-background/80 rounded border">
                        <p className="text-[10px] text-muted-foreground uppercase">Statutory Ded.</p>
                        <p className="font-bold text-red-500">-{currency.symbol}{calculatedProcess.totalDeductions.toLocaleString()}</p>
                      </div>
                      <div className="p-2 bg-emerald-500/10 rounded border border-emerald-500/20">
                        <p className="text-[10px] text-emerald-600 dark:text-emerald-400 uppercase font-bold">Net Payout</p>
                        <p className="font-extrabold text-emerald-500">{currency.symbol}{calculatedProcess.net.toLocaleString()}</p>
                      </div>
                    </div>
                    <p className="text-[10px] text-muted-foreground text-center">
                      Includes Basic ({currency.symbol}{calculatedProcess.basic.toLocaleString()}) + HRA 40% ({currency.symbol}{calculatedProcess.hra.toLocaleString()}) + PF ({currency.symbol}{calculatedProcess.pf.toLocaleString()})
                    </p>
                  </div>
                )}
                ...
                <div className="flex justify-end gap-2 pt-2">
                  ...
                  <button type="submit" className="px-4 py-2 bg-primary text-white rounded-md text-sm font-semibold hover:opacity-90">
                    Run & Process Payslip
                  </button>
                </div>
              </form>
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="w-full max-w-md rounded-2xl bg-card border p-6 space-y-4 shadow-2xl">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-lg text-foreground">1-Click Batch Payroll Processing</h3>
                  <p className="text-xs text-muted-foreground">Automated payroll run for all active employees.</p>
                </div>
              </div>

              <form onSubmit={handleProcessBatchPayroll} className="space-y-4">
                <div className="p-3 bg-muted/40 rounded-xl border border-border/50 space-y-1.5 text-xs text-muted-foreground">
                  <p className="font-semibold text-foreground flex items-center gap-1.5">
                    <Sparkles className="size-3.5 text-primary" /> What will happen automatically:
                  </p>
                  <ul className="list-disc list-inside space-y-1 pl-1">
                    <li>Auto-calculates Basic, HRA (40%), Allowances (10%)</li>
                    <li>Auto-computes statutory PF (12%), ESIC (0.75%) & TDS</li>
                    <li>Renders branded PDFs using active Payslip Template</li>
                    <li>Archives salary documents to Document Vault for {employees.length} active employees</li>
                  </ul>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    ...
                    <select value={batchMonth} onChange={e => setBatchMonth(e.target.value)} className="w-full h-10 px-3 text-sm rounded-md border bg-background">
                      <option value="1">January</option>
                      <option value="2">February</option>
                      <option value="3">March</option>
                      <option value="4">April</option>
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-muted-foreground uppercase">Year</label>
                    <select value={batchYear} onChange={e => setBatchYear(e.target.value)} className="w-full h-10 px-3 text-sm rounded-md border bg-background">
                      <option value="2025">2025</option>
                      <option value="2026">2026</option>
                      <option value="2027">2027</option>
                    </select>
                ...
                <div className="flex justify-end gap-2 pt-2">
                  ...
                  <button
                    type="submit"
                    disabled={batchLoading}
                    className="px-4 py-2 gradient-brand text-white rounded-md text-sm font-semibold flex items-center gap-1.5 shadow-elegant hover:opacity-90 disabled:opacity-50"
                  >
                    {batchLoading ? (
                      <><Loader2 className="size-4 animate-spin" /> Processing All Profiles...</>
                    ) : batchSuccess ? (
                      <><CheckCircle2 className="size-4 text-emerald-300" /> Disbursed Successfully!</>
                    ) : (
                      <><Zap className="size-3.5" /> Run Batch for {employees.length} Employees</>
                    )}
                  </button>
                </div>
              </form>
