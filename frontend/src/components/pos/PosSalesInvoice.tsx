...
export function PosSalesInvoice({ initialDocType = "TAX_INVOICE", editingInvoice, onCancel, onSaved, onConvertToOrder }: PosSalesInvoiceProps = {}) {
  ...
  const handleUpdateWalkInName = (name: string) => {
    ...
    setCustomers((prev) =>
      prev.map((c) => {
        if (c.id === selectedCustomer || c.id === "walk-in" || (selectedCustomer && selectedCustomer.startsWith("walk-in"))) {
          return { ...c, name: val };
        }
        return c;
      })
    );
    if (!selectedCustomer || selectedCustomer === "walk-in" || selectedCustomer.startsWith("walk-in")) {
      const walkInObj = {
        id: selectedCustomer || "walk-in",
        name: val,
        customer_type: "Walk-in",
        type: "Retail",
      };
      setCustomers((prev) => {
        if (!prev.some((c) => c.id === walkInObj.id)) {
          return [walkInObj, ...prev];
        }
        return prev.map((c) => (c.id === walkInObj.id ? { ...c, name: val } : c));
      });
      if (!selectedCustomer) {
        setSelectedCustomer(walkInObj.id);
      }
          ...
          <div className="bg-white p-1.5 sm:p-2 rounded-2xl border border-slate-200/80 shadow-2xs space-y-2">
            <div className="flex items-center justify-between pb-0.5">
              ...
              <div className="flex items-center gap-1.5">
                ...
                >
                  <User className="size-3.5" /> + Walk-in
                </button>
                <button
                  type="button"
            ...
            <div className="space-y-3">
              ...
              {activeCustomerObj ? (
                activeCustomerObj.id === "walk-in" || activeCustomerObj.customer_type === "Walk-in" || (!activeCustomerObj.phone && !activeCustomerObj.gst_number) ? (
                  /* Inline Walk-in Customer Field */
                  <div className="bg-amber-50/70 border border-amber-200 rounded-2xl p-3 space-y-2 transition-all">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <div className="size-7 rounded-lg bg-amber-500 text-white font-black text-xs flex items-center justify-center shadow-2xs">
                          🚶
                        </div>
                        <div>
                          <div className="flex items-center gap-1.5">
                            <span className="font-bold text-slate-900 text-xs">Walk-in Customer</span>
                            <span className="text-[10px] bg-amber-100 text-amber-800 font-bold px-1.5 py-0.2 rounded border border-amber-300 uppercase">
                              Guest Mode
                            </span>
                          </div>
                          <span className="text-[10px] text-slate-500 font-medium">Quick billing without registration</span>
                        </div>
                      </div>
                      <button
                        type="button"
                        ...
                        className="px-2 py-1 text-[11px] font-bold text-slate-500 hover:text-rose-600 hover:bg-rose-50 rounded-lg border border-slate-200/80 transition-colors cursor-pointer"
                      >
                        Change Party
                      </button>
                    </div>
                    ...
                    <div className="pt-1">
                      <label className="text-[11px] font-bold text-slate-700 block mb-1 flex items-center justify-between">
                        <span>Customer / Walk-in Name:</span>
                        <span className="text-[10px] text-slate-400 font-normal">Directly editable</span>
                      </label>
                      <input
                        type="text"
                        placeholder="e.g. Walk-in Customer, Ramesh, Sneha"
                        value={activeCustomerObj.name || ""}
                        onChange={(e) => handleUpdateWalkInName(e.target.value)}
                        className="w-full h-9 bg-white border border-amber-300 focus:border-indigo-500 rounded-xl px-3 text-xs font-bold text-slate-800 outline-none focus:ring-2 focus:ring-indigo-500/20 shadow-2xs"
                      />
                      <p className="text-[10px] text-slate-400 mt-1">
                        * Saved with sales & CRM invoices. Phone and address are optional.
                      </p>
                    </div>
                  </div>
                ) : (
                  /* Registered CRM Customer Profile */
