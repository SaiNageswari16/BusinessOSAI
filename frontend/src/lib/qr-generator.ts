...
export function generateQRCodeSVG(text: string, size = 220): string {
  ...
  try {
    ...
    const margin = 4; // ISO-18004 4-module quiet zone
    ...
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${size} ${size}" width="${size}" height="${size}" shape-rendering="crispEdges"><rect width="100%" height="100%" fill="#ffffff"/><path d="${path}" fill="#000000"/></svg>`;
