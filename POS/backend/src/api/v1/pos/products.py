...
async def list_products(
    ...
    from src.models.erp import ProductCategory, Brand
    stmt = (
        select(Product)
    ...
    # Enrich with category name using safe lazy access
    out = []
    for p in products:
        try:
        ...
        d = POSProductResponse.model_construct(
            id=p.id, tenant_id=p.tenant_id, name=p.name, brand=brand_name,
...
async def create_product(
    ...
    # Sanitize category_id: reject fake 'all' string or non-UUID values
    raw_cat_id = data.get("category_id")
    if raw_cat_id is not None:
        try:
            import uuid as _uuid
            parsed = _uuid.UUID(str(raw_cat_id))
            # Verify the category actually exists in DB
            from src.models.erp import ProductCategory
            cat_check = await db.execute(select(ProductCategory).where(ProductCategory.id == parsed))
            if not cat_check.scalar_one_or_none():
                data["category_id"] = None
        except (ValueError, AttributeError):
            data["category_id"] = None

    # Always ensure status is set to active for new products
